...
func main(){
	...
	for {
		...
		if err != nil {
			...
			fmt.Println("Now Block:",string(body2))
                        if err2 != nil {
                                fmt.Println(err2)
                                time.Sleep(1 * time.Second)
                        }
			fmt.Println(core_arg.Config)
			if(string(body)!="no block hash"){
				...
				blockSync(*core_arg , reqBlock)
					...
					log.Println(reqBlock.Transaction[i])
