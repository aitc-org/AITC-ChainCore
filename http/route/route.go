...
type BroadMulti struct {
    AllParam []types.TransactionJson `json:"allparam"`
} 
...
func Router(coreStruct *types.CoreStruct){
	...
	Signmulti := func (res http.ResponseWriter, req *http.Request){
                //res.Header().Add("Access-Control-Allow-Origin","*")
                //res.Header().Add("Access-Control-Allow-Methods", "GET,PUT,POST,DELETE,OPTIONS")
                EnableCors(&res)
                b, _ := ioutil.ReadAll(req.Body)
                ...
                exTrans := transaction.Transactionmultitest(signbody)
                res.Header().Set("content-type", "application/json")
                fmt.Println("SignTransaction")

                rawsigninfo := SignMulti{exTrans}
                signinfo, _ := json.Marshal(rawsigninfo)
                res.Write([]byte(string(signinfo)))
        }
...
*/
		EnableCors(&res)
		rawsigninfo :=PostTestStruct{"signTransaction","ccc"}
	...
	http.HandleFunc("/signmulti", Signmulti)
